/*
AURELIO BONILLA ALMARCHA 
JAVIER ORTS GARCÍA
GUILLERMO GONZÁLEZ RICHART
RAÚL PÉREZ RUIZ
*/
...
/*
#########################################
CÓDIGO DEL EQUALIZATION PARALELIZADO
#########################################
void equalization(cv::Mat &in, cv::Mat &out, float black, float white, float saturation)
{
	auto start = high_resolution_clock::now();
	...
	// Applying equalization to each channel separately in parallel
	#pragma omp parallel for
	for (int i = 0; i < 3; ++i)
	{
		if (i == 2) {
			// Normalize the value channel between black and white levels
			cv::normalize(channels[i], channels[i], black, white, cv::NORM_MINMAX);
		}
		else if (i == 1) {
			// Increase the saturation channel
			channels[i] *= (1 + saturation);
		}
	}

	// merge channels back to HSV and convert to output format
...
//#########################################
//CÓDIGO DEL EQUALIZATION SIN PARALELIZAR
//#########################################
void equalization(cv::Mat &in, cv::Mat &out, float black, float white, float saturation)
{
	auto start = high_resolution_clock::now();
...
/*
#########################################
CÓDIGO DEL DENOISE SIN PARALELIZAR
#########################################
...
/*
#####################################################
CODIGO PARALELIZADO COMENTADO PORQUE NO ES RENTABLE
######################################################
...
void debayer(LibRaw* processor, cv::Mat &out)
{
    ...
    std::vector<ushort> bayerData(width * height); // Buffer for bayer data

    #pragma omp parallel for collapse(2) shared(bayerData) schedule(dynamic)
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            {
                bool red = x % 2 == 0;
                bayerData[idx] = rgbg[red ? 0 : 1];
            }
            else
            {
                bool green = x % 2 == 0;
                bayerData[idx] = rgbg[green ? 3 : 2];
            }
        }
    }
...
//###########################################
//CÓDIGO DE  ENHANCEDETAILS PARALELIZADO
//###########################################
void enhanceDetails(cv::Mat &in, cv::Mat &out, float sigma, float amount)
{
	...
	float* pIn, *pBlur;
	// Applying enhancement to each pixel separately in parallel
	#pragma omp parallel for
...
//###########################################
//CÓDIGO DE  BLOOM PARALELIZADO pa mañana
//###########################################
void bloom(cv::Mat &in, cv::Mat &out, float sigma, float threshold)
{
	auto start = high_resolution_clock::now();
	...
	// Convert the computed mask to a 3-channel image and 16-bit
...
/*
//###########################################
//CÓDIGO DE  BLOOM SIN PARALELIZAR
//###########################################
void bloom(cv::Mat &in, cv::Mat &out, float sigma, float threshold)
{
	auto start = high_resolution_clock::now();
...
//###########################################
//CÓDIGO DE  GAMACORRECTION PARALELIZADO
//###########################################
void gammaCorrection(cv::Mat& in, cv::Mat& out, float a, float b, float gamma)
{
	auto start = high_resolution_clock::now();
	...
	// Applying gamma correction to each pixel separately
	#pragma omp parallel for 
	for (int i = 0; i < in.rows; ++i)
	{
		unsigned short* pIn = in.ptr<unsigned short>(i);
		unsigned short* pTmp = tmp.ptr<unsigned short>(i);

		for (int j = 0; j < in.cols; ++j)
		{
			pTmp[j * 3] = a * curve[pIn[j * 3]] + b;
			pTmp[j * 3 + 1] = a * curve[pIn[j * 3 + 1]] + b;
			pTmp[j * 3 + 2] = a * curve[pIn[j * 3 + 2]] + b;
		}
	}
...
/*
###########################################
CÓDIGO DE  GAMACORRECTION SIN PARALELIZAR
###########################################
void gammaCorrection(cv::Mat& in, cv::Mat& out, float a, float b, float gamma)
{
	auto start = high_resolution_clock::now();
...
/*
###########################################
CÓDIGO DE  COLORBALANCE PARALELIZADO
###########################################
void colorBalance(cv::Mat& in, cv::Mat& out, float percent) {
    ...
    // Applying color balance to each channel separately
    #pragma omp parallel for
    for (int i = 0; i < 3; ++i)
    {
...
//###########################################
//CÓDIGO DE  COLORBALANCE SIN PARALELIZAR
//###########################################
...
//###########################################
//CÓDIGO DE  SCREENMERGE PARALELIZADO
//###########################################
void screenMerge(cv::Mat &in1, cv::Mat &in2, cv::Mat &out)
{
    auto start = high_resolution_clock::now();
    ...
    // Apply the screen mode merge to each pixel separately in parallel
    #pragma omp parallel for
...
/*
//###########################################
//CÓDIGO DE  SCREENMERGE SIN PARALELIZAR
//###########################################
void screenMerge(cv::Mat &in1, cv::Mat &in2, cv::Mat &out)
{
    auto start = high_resolution_clock::now();
